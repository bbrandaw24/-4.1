...
| v15.4.0 | 2026-08-31 | （见提交） | feat(web)：**新增第四个板块「数据屏幕」**（科技感大数据大屏）。新增 `web/dashboard.css`：深色沉浸式主题（`#020617` 径向渐变底 + 霓虹蓝 `#0ea5e9`/`#22d3ee` 描边角标 + 发光文字），进入该视图时 `body.dashboard-active` 隐藏常规面板并让 `.shell` 全宽铺满。布局：顶部标题栏（实时时钟）→ KPI 四卡（总地块数 / 在线传感器 / 平均土壤湿度 / 平均空气温度，越界自动变色提示）→ 三列主体（左：地块在线状态 + 派生告警流；中：Canvas 实时数据脉动环——同心脉冲圆 + 12 条旋转辐条 + 每地块一个数据点，半径映射该地块土壤湿度，中心显示湿度均值；右：土壤六项指标详情 + 最近遥测时间流）→ 底部三卡（光照 / 空气湿度 / pH）。数据源全部为真实遥测：复用 `state.allDevices`，由 `latestPayload(device, kind)` 取每个地块 `history[]` 中时间戳最新的 soil/climate payload，跨地块求均值；告警由真实阈值派生（湿度 <40% 或 >65%、气温 <18°C 或 >28°C、设备 90 秒无上报判定离线）。新增 `escapeHtml()` 对地块名/作物名做转义；`dashPlotName/dashPlotCrop` 统一读取 `device.plot`（此前易误写为 `device.name`）。路由 `setRoute` 新增 `dashboard`，`refresh()` 成功后同步调用 `renderDashboard(false)`，仅在面板可见时渲染。PWA 缓存 `smartagri-v7` → `v8`，`app.js` 版本串 `v15.3.3` → `v15.4.0`
| v15.5.1 | 2026-09-01 | `5d16872` | fix(simulator)：**传感器全部 "--" 死循环修复**——根因：模拟器调 API 持续 HTTP 401（容器 08-31 08:52 重建后 token 失效），但启动只登录一次、主循环遇 401 只打 `LOGGER.warning` 不重登 → 永久卡死 → 无法 `list_sensors`/`sync_devices` → 新地块传感器从未收到遥测 → `sensors[].value=None` → 前端渲染 `--。修法（`services/simulator/app/main.py`）：① `ApiClient.__init__` 加 `_username/_password`，`login()` 缓存凭据；② 新增 `_relogin()` 私有方法（401 后重新认证）；③ `list_devices`/`list_sensors` 用 `for attempt in range(2)` 包装，401 时第一次尝试 `_relogin()` 后重试，并把 `RuntimeError`（来自 `_headers()` token 空）加入 except；④ 主循环新增 `last_login_at` + 每 600s（`LOGIN_REFRESH_SECONDS` env 可调）定期 `api.login()` 续期。模拟器 `rebuild + up -d` 即恢复，API/web 不动。实测三个地块（苹果园/梨园/橘园）15 个传感器全部恢复真实遥测：土壤温度 22.83°C、pH 6.18、NPK 126/52/...、空气湿度 66.62%、电导率 1.066；前端设备页 25 张 `.sensor-card` 全部显示数值（非"--"），状态"已连接"，最近上报 16:22 |
| v15.6.0 | 2026-09-02 | `26a24fc` | feat(api/web)：**地块按账户隔离（多租户）**——每块地归属且仅归属一个账户，农户/游客只能看到自己创建或被分配的地块，管理员可见全部。后端（`services/api/app/main.py`）：① `custom_plots` 表加 `owner_user_id` 列（ALTER 迁移 + 存量行回填 admin）；② 新增归属 helper：`_plot_owner`（registry→DB→内置回退）、`_accessible_device_ids`（管理者返回 None=全部，否则按 owner 过滤）、`_can_access_plot`/`_plot_access_error`（统一 403 `plot_forbidden`）；③ 全部 14 个 device 级路由加归属校验（sensors 读写、telemetry、pump、irrigation-rules、alerts 等），`GET /devices` 响应带 `scope: all|own`、每项带 `owner_user_id`/`owner_label`；④ 内置 3 地块启动即播种进 registry 并归属 admin（`_seed_builtin_plots_into_registry`，修复重启后内置地块消失且与模拟器互相等待的死锁——播种调用必须放模块末尾，依赖 `create_sensor`/`utc_now` 先定义）。前端（`web/app.js`+`sensor.css`）：空地块显示引导卡「当前账户还没有地块」+「+ 添加我的地块」按钮；管理者视角每个地块组显示归属徽标（内置/用户名）。PWA 缓存 `smartagri-v8` → `v9`，`app.js`/`sensor.css`/`index.html` 版本串 → `v15.6.0`。实测：admin 见 5 地块全归内置、新农户初始空列表、自建地块 owner=本人、越权访问内置地块 403、模拟器（admin 身份）正常给农户地块推遥测（5/5 传感器有值）、浏览器空态引导与归属徽标正常 |
| v15.7.0 | 2026-09-02 | `7789287` | feat(api/web)：**作物目录规范化**——crop 从自由文本改为受控目录（为后续生长进度/作物差异化告警/PK 评分提供稳定维度）。后端（`services/api/app/main.py`）：① 新增 `CROPS` 常量目录（18 种常见作物：果树/浆果/茄果/瓜类/叶菜/大田/油料 7 类，各带 `growing_days` 生长周期、`soil_moisture`/`air_temp`/`ph` 适宜区间、`npk` 参考值——当前作元数据，供后续功能使用）；② `normalize_crop()` 按标准名/别名/key 归一化（如「桔子」→「橘子」、「包谷」→「玉米」）；③ 新增 `GET /api/v1/crops` 目录接口（18 项，含全部参考区间）；④ `POST /devices` 创建地块时 crop 强制校验：不在目录返回 400 `crop_not_in_catalog` + 可选清单；存量地块 crop 不校验读取、原样显示（兼容）。前端（`web/app.js`+`index.html`）：添加地块对话框作物下拉从硬编码 9 项改为 `populateCropSelect()` 动态拉取目录（单一数据源，失败回退静态选项），移除「其他」自由项。PWA 缓存 `smartagri-v9` → `v10`，版本串 → `v15.7.0`。实测：目录 18 种完整、非法作物（榴莲）400 且附可选清单、别名「桔子」归一化为「橘子」、合法创建成功、存量地块（苹果园/梨园/橘园+用户自建 3 块）全部兼容、浏览器下拉 18 项来自 API |
| v15.8.0 | 2026-09-02 | `ec98c9d` | feat(api/web)：**AI 农场管家**（每账户自动化"管家"，用户设定权限与阈值，agent 分析执行）——架构原则：**规则引擎做决策（确定性/免费/可预期），LLM 留作后续解释与工单润色**，不做热路径。后端（`services/api/app/main.py`）：① 新表 `steward_config`（按 `owner_user_id` 隔离：auto_pump_enabled/moisture_threshold_pct 10-90/pump_duration_min 1-60/auto_tickets_enabled）+ `steward_actions`（时间线：pump_on/pump_off/ticket，带 reason/detail）；② `GET/PUT /api/v1/steward/config`（写入需 `manage_rules`，farmer 本就有此权限=管理自家管家，guest 403）+ `GET /api/v1/steward/actions`（按 `_accessible_device_ids` 过滤，manager 见全部）；③ 后台守护线程 `steward_loop`（30s 一轮）：湿度跌破阈值 → `_publish_pump_command(..., source="steward")` 自动开泵并写时间线「检测到土壤湿度 X% 跌破阈值 Y%，自动开启水泵 N 分钟」；运行达时长自动关泵；高温（>30°C）高湿（>80%）自动生成病虫害防治工单（建议文本）；每地块每动作 5 分钟冷却、工单 30 分钟冷却防刷。前端（`web/app.js`+`index.html`+`sensor.css`）：设备页新增「🤖 AI 农场管家」面板——左配置卡（自动灌溉开关/湿度阈值/泵时长/工单开关，manage_rules 可编辑否则只读）+ 右管家时间线（💧自动开泵/✅自动关泵/🛡️防治工单，时间线流式渲染）。PWA 缓存 `smartagri-v10` → `v11`，版本串 → `v15.8.0`。实测：默认配置返回、保存生效、**湿度 38%<阈值 50% 自动开泵 3 条 + 1 分钟后自动关泵 3 条 + 泵回 standby**、多租户隔离（admin 管家只动 owner=1 内置 3 块、owner=5 地块不受影响）、farmer 读写自己配置 200（manage_rules 设计如此）、阈值越界 400、浏览器面板配置回显+6 条时间线正常 |
| v15.9.0 | 2026-09-02 | `99c1bde` | feat(api/web)：**农场报告**——报告卡片+同作物 PK 排行（成长报告/认养/排行榜公共地基的前两层）。后端（`services/api/app/main.py`）：`GET /devices` 响应 `item.plot.created_at` 字段（从 `custom_plots` 读取；内置/无记录地块为 `null`）。前端（`web/app.js`+`index.html`+`sensor.css`）：新 nav 板块「🌱 农场报告」——① 报告卡片（绿色封面：作物徽标+地块名+**健康度环（满分 100，按湿度 40%/气温 30%/pH 20%/离线 10% 偏离作物目录参考区间程度扣分）**+**生长进度条（age_days ÷ growing_days，阶段标签 播种/幼苗/生长/成熟/已成熟）**+ 作物元信息）；② 实时指标卡（湿度/气温/pH 当前值 vs 适宜区间，色标适宜/偏离）；③ 关键事件（管家动作时间线前 5 条）；④ 一键复制分享文本到剪贴板；⑤ **🏆 同作物健康度 PK 排行**（按作物分组、各组内按健康度排序、🥇🥈🥉 徽章、榜首「最稳农夫」金标、点击切换报告地块）。修复了首屏空白 bug（`refresh()` 成功后若 reports 面板可见则重渲染，确保 `?view=reports` 直开也能拿到数据）。PWA 缓存 `smartagri-v11` → `v12`，版本串 → `v15.9.0`。实测：admin 视图选苹果地 → 健康度 52（湿度/温度/pH 全偏离目录适宜区间）、进度 0%/170 天/播种期、3 项指标偏离提示、PK 5 组（苹果/草莓/番茄/梨/橘子，梨园 21 分居榜首「最稳农夫」）、分享按钮可复制、整体视觉效果如截图 |
| v15.11.0 | 2026-09-02 | `bcc20af` | feat(api/web)：**认养玩法升级（时间倍率 / 收获积分 / 积分排行榜 / 一物一码溯源）**。后端（`services/api/app/main.py`）：① `adoptions` 表加 `time_scale`（默认 1，**1 分钟 = 1 天生长**）+ `harvest_count`；新建 `harvests` 表（owner/device/crop/nickname/health_score/grade/grade_label/points/harvested_at）；② `_adoption_growth()` 按倍率计算进度/剩余天数/成熟态，`_plot_health_score()` 后端健康度评分（与前端报告卡同一算法：湿度 40/气温 30/pH 20/离线 10），`_grade_for()` 评级（≥80 优秀 ×2 / ≥60 良好 ×1.5 / ≥40 及格 ×1 / 其余不及格 ×0.5），`_CROP_BASE_POINTS` 各作物基础分（果树 140-160/大田 110-150/叶菜 70）；③ 新 API：`PATCH /api/v1/adoptions/<id>`（time_scale 1-60，管理者可加速）、`POST .../harvest`（仅成熟可收，按健康度评级发积分，收获后自动重新播种）、`GET /api/v1/adoptions/points`（**我的积分**：按账户隔离）、`GET /api/v1/adoptions/leaderboard`（**积分排行榜**：跨账户按作物分组按累计积分排序）、`GET /api/v1/trace/<device_id>`（**免鉴权公开溯源**）。前端（`web/app.js`+`index.html`+`sensor.css`+`trace.html` 新文件）：认养卡升级（倍率化生长进度 + 收获次数 + ⏩ 加速徽标 + 管理者倍率下拉 1/2/5/10/30/60× + 🌾 收获按钮成熟时发光 + 🔗 溯源码链接）；认养页新增「🏆 我的收获积分」卡 +「📈 认养积分排行榜」（按作物分组、🥇🥈🥉、榜首金标）；新溯源页 `trace.html?code=adopt-xxx`（免登录：深色科技风 + 生长进度/健康度/收获履历/AI 管家记录 + qrcodejs 二维码，CDN 失败降级文本链接）。PWA 缓存 `smartagri-v13` → `v14`，版本串 → `v15.11.0`。实测：1× 时 1 分钟 = 1 天、60× 加速进度瞬到 100% 成熟、收获大豆 +100 / 葡萄 +140（中文作物名积分解析修复）、未成熟收获 409、积分按账户隔离（admin 见 140）、排行榜按积分排序、溯源公开接口免鉴权、浏览器认养页 2 卡全成熟 + 积分卡 + 排行榜 + 溯源页二维码渲染 |
| v15.12.0 | 2026-09-02 | `f5d00f7` | feat(web)：**数字孪生 3D 农场**（Three.js + 遥测驱动虚拟作物）。新 nav 板块「🎮 3D 农场」：① Three.js r128（cdnjs 单文件引入，**OrbitControls CDN 404 → 自实现轨道控制**：拖拽旋转/滚轮缩放/空闲自动旋转，约 30 行，不再依赖第二个 CDN）；② 每个地块一棵虚拟作物（土壤堆+茎+叶球+果实），**茎高/叶数随生长进度**（认养地块走倍率化进度）、**叶色随健康度**（≥80 深绿/60-79 黄绿/40-59 黄/<40 橙，含轻微自发光）、成熟期长出红色果实、**认养地块加紫色光环**；③ 深色科技蓝场景（雾效/网格地面/方向光+青蓝补光/软阴影），环形摆放自动布局；④ **点击作物射线拾取 → 右侧信息面板**（地块名/作物/归属/健康度/进度/湿度/气温/pH/在线状态 + 指标偏离说明）；⑤ 数据每 30 秒随 refresh 同步更新（crops 增删/颜色/高度实时刷新）；⑥ 修复两处首屏 bug：`?view=farm3d` 直开 crops=0（refresh 钩子补 build）+ OrbitControls 404 中断 init。PWA 缓存 `smartagri-v14` → `v15`，版本串 → `v15.12.0`。实测：10 个地块全部生成 3D 作物、10 在线、信息面板显示真实遥测（湿度 38%/气温 21.56°C/pH 6.2）、截图存档 |
| v15.12.1 / v15.12.2 | 2026-09-02 | `5f55c7a` | fix(api/web)：**3D 农场/报告生长进度异常修复**——两层根因：① 内置 3 地块（苹果园/梨园/橘园）`plot.created_at` 为 None（PLOT_META 无播种时间、custom_plots 无行）→ `plotProgress` 返回 null → 3D 作物全部 0% 小苗。修复：PLOT_META 加固定演示播种时间 `2026-08-01`，`_load_plot_created_at()` 回退 PLOT_META，`_seed_builtin_plots_into_registry`/`_create_owned_plot` 播种时带 created_at；② 前端 `renderFarm3D` 从不加载作物目录（reportCrops 空 → `cropFound:false` → 进度 null→0）与认养列表（认养地块 growth 缺失）→ 改为 async，build 前先 `ensureReportCrops()` + 拉取 `/api/v1/adoptions`。实测：18 种作物目录加载、进度分布正确（苹果园 19%/梨园 18%/橘园 15%、认养葡萄 27%/大豆 30%、新地块 0%）、信息面板显示真实进度与遥测 |
| v15.13.0 | 2026-09-02 | `1b78b33` | feat(api/web)：**认养积分榜改为「农户积分总榜」（不含管理员）**。去掉按作物分组、改为单总榜按积分排名。后端：`adoptions/leaderboard` 跨 users.db 查 `role='manager'` 的 uid 集合（含兜底内置 admin id=1），SQL `WHERE owner_user_id NOT IN (...)` 排除管理员；响应结构从 `{groups:{crop:[...]}}` 改为 `{entries:[...]}` + `scope: "farmers"`。实测：admin 葡萄 140 分被过滤，仅「老马 100 分」上榜（owner=5 是普通农户） |
| v15.13.1 | 2026-09-02 | `96523ef` | fix(web)：**农场报告页排行改为「农户积分榜」**（替代同作物健康度 PK）——`renderRanking` 从本地按作物分组健康度排序改为调 `/api/v1/adoptions/leaderboard`（不含管理员），渲染单总榜（🥇🥈🥉/收获次数/均健康/积分），移除"点击排行切地块"交互；标题「🏆 农户积分榜（不含管理员 · 按积分）」。实测：老马 210 分居首、健康度 PK 已完全移除 |
| v15.10.0 | 2026-09-02 | `ecef974` | feat(api/web)：**认养农场**——从作物目录选品种，平台创建专属地块并颁发认养证书。后端（`services/api/app/main.py`）：① 新表 `adoptions`（owner_user_id/device_id/crop/nickname/adopted_at，多租户隔离）；② 提取公共 helper `_create_owned_plot(device_id, name, crop, owner)`（registry 建条目 + _save_custom_plot + seed 5 传感器 + _publish_new_plot，复用于 v15.7.0 自建地块/本次认养）；③ `POST /api/v1/adoptions` 校验 crop 在目录（不在返回 400 `crop_not_in_catalog` + 可选清单）、自动生成 `adopt-xxxxxxx` 设备 id、owner=当前用户、插入 adoptions；④ `GET /api/v1/adoptions`（admin 见全部，其他见自己）；⑤ `DELETE /api/v1/adoptions/<device_id>` 解除认养（删地块+传感器+adoptions 行）。前端（`web/app.js`+`index.html`+`sensor.css`）：新 nav 板块「🧑‍🌾 认养农场」——认养表单（作物下拉 18 种+昵称输入+认养按钮）+ 我的认养卡片网格（作物徽标/昵称/认养日期/健康度/进度/在线状态 + 查看报告按钮跳报告页 + 解除认养按钮）。PWA 缓存 `smartagri-v12` → `v13`，版本串 → `v15.10.0`。实测：合法认养返回 201 证书、非法作物（榴莲）400、列表展示认养记录、模拟器 45s 内给新认养地块推 5/5 传感器、浏览器认养页渲染（18 种作物下拉 + 2 张认养卡 + 查看报告跳转 reports 并自动选中该地块 + 7 组 PK 排行），整体视觉效果如截图 |
