...
| v16.1 | 2026-09-01 | `f19ee3a` | feat(web)：**数据大屏板块改造**——导航第 5 项「数据屏幕」(`data-view="dashboard"`) 按 futuristic HUD 提示词重构：顶部红色 LED 跑马灯（如需帮助请拨打 027-67883697）、发光金属质感主标题「智慧农业大数据平台」、2×3 模块化网格（实时监控视频窗 / 农田种植数据预警表（红黄绿状态标签）/ 区域种植分布图（脉冲光点节点 + 流动连线）/ 24 小时环境监测折线 / 农场实景 + 6 项环境指标卡 / 土地墒情多线趋势）+ 底部 4 卡（人员出勤率曲线 / 土地使用面积分组柱状 / 农作物种类占比环形（总亩数 3500）/ 农作物生长进度条）；保留 app.js 真实数据接线（KPI / 地块在线 / 告警 / 遥测 / canvas 脉动均继续驱动）；图表与地图全部本地 SVG 内联、图标改用本地 `icons.js`（新增 14 个 dashboard 图标，去掉 unpkg lucide CDN 依赖）。PWA 缓存 `smartagri-v8` → `v9`，`dashboard.css` → `v16.1` |
| v16.2 | 2026-09-01 | `0ae5320` | feat(theme)：**新增第三种皮肤「科技蓝 HUD」**——由数据大屏风格下沉为全站可选主题。`theme.css` 新增 `:root[data-theme="hud"]` Design Token（深色科技蓝背景 `#020617→#0f172a`、霓虹青蓝 `#06b6d4 / #0ea5e9` 高亮、玻璃拟态、发光数字/辉光），并把原 dark 的全部组件规则选择器扩展为同时匹配 hud（逗号拼接保留特异性），概览/趋势/设备/智能体等板块均可套用该皮肤；`enhance.js` 主题按钮改为三态循环（自然清新 → 深色大屏 → 科技蓝 HUD），同步按钮图标/文字/`meta theme-color`；`v16-ui.js` 设置抽屉主题选项新增「科技蓝 HUD」chip；`login.html` 主题初始化接受 hud。PWA 缓存 `smartagri-v9` → `v10`，`theme.css`/`enhance.js` → `v16.2` |
| v16.3 | 2026-09-02 | `c66b7e0` | feat(web)：**移除数据大屏板块 + 静态资源全面本地化**——① 按需求删除导航「数据屏幕」(`data-view="dashboard"`)：index.html 移除导航项与整段大屏 section，app.js 移除 `renderDashboard`/`updateDashboardClock`/`startDashboardCanvas` 及其定时器接线，路由收敛为 7 项（概览/趋势/设备/智能体问答/农场报告/认养农场/3D 农场）；② 本地化：`three.min.js`（r128，603KB）落库替换 cdnjs CDN，icons.js 补齐 6 个缺失图标（wind/bar-chart-2/box/cctv/heart-handshake/image/line-chart），login.html 移除被浏览器拦截的 Google Fonts 外链；③ PWA 缓存 `smartagri-v15` → `v16`（APP_SHELL 移除 dashboard.css、新增 icons.js/three.min.js），前端版本串 → `v15.13.0`；④ 线上重建容器验证：无 dashboard 残留、icons.js/three.min.js HTTP 200、无 unpkg/cdnjs/googleapis 引用 |
| v16.4 | 2026-09-02 | `see git log` | feat(web+api+sim)：**3D 农场作物模型重制 + 取消默认地块**——① 3D 农场（farm3d）作物按品类区分外观：苹果/梨/橘=果树（锥形树干+椭球树冠+果实干挂枝头）、草莓/其他=低矮叶丛、番茄=支架藤蔓+垂挂果、麦稻谷=秆簇+成熟金穗、玉米=高秆+弧叶+棒穗；土壤改梯形耕地（田垄三行），整株随风摆动（相位错开），生长进度驱动整体缩放、健康度驱动叶色、成熟后果实显现，认养光环随田块加大；② 农户新开不再自带三个默认地块：模拟器 `DEFAULT_PROFILES` 置空（新部署空农场起步，仍可用 `SIM_DEVICES` 环境变量播种演示地块），API 移除 `PLOT_META` 内置地块保护（苹果园/梨园/橘园改为普通可删地块），存量线上三地块由部署脚本调用 DELETE 接口清理；③ app.js 清除大屏残留死代码（`DASH_SENSOR_LABELS`/`dashAvg`/`dashNum`/`latestPayload`/`isDeviceOnline`/`dashPlotName/Crop`）；④ 版本串 → `v15.14.0`，PWA 缓存 `smartagri-v16` → `v17` |
| v16.5 | 2026-09-02 | `8a364cc` | fix(api+sim)：**恢复被覆盖的认养/积分榜后端 + 空农场起步落地**——v16.4 曾用滞后基线覆盖 main.py，导致 `/api/v1/adoptions` 等 7 条认养路由、收获积分、农户积分总榜、地块按账户隔离等 v15.6~v15.13.2 后端整批 404；从提交 `b07f3093` 完整恢复 api/main.py（105KB）、simulator/main.py 与 test_sensors.py（前端经 blob sha 校验无损）；在恢复版上重做「无默认地块」：移除 API 启动时的三内置地块自动播种（`_seed_builtin_plots_into_registry` 调用点删除，PLOT_META 保留作地块 id 预留与播种时间回退），模拟器 `DEFAULT_PROFILES` 置空——新部署空农场，网页添加地块或认养作物即自动创建专属地块并上线；线上实测：认养→专属地块创建→解除认养自动清理全链路 200；另修「解除认养 HTTP 403」：权限改以认养证书归属为准（`_is_manager` 或证书 owner 本人即可解除），地块已丢失的陈旧证书不再因 orphan 回落误拒合法主人 |
| v16.6 | 2026-09-02 | `see git log` | fix(web+api)：**农场报告页修复 + 右栏改积分排行榜**——① 右侧栏由「同作物健康度 PK」改为「🏆 农户积分排行榜」（复用 `/api/v1/adoptions/leaderboard`，金银铜牌 + 收获次数 + 均健康 + 积分）；② 报告指标卡 NaN 容错：遥测缺失（如 API 重启后瞬窗）时 `rangeLine` 以 `Number.isFinite` 判空显示「--」，不再渲染 "NaN%" 乱码（即用户截图中的空指标卡）；③ `owner_label` 修正：内置地块取消后 admin 名下地块由误导性的「内置」改标「管理员」；版本串 → `v15.15.0`，PWA 缓存 `smartagri-v17` → `v18` |
...
| v16.7 | 2026-09-02 | `a4ba15f` | feat(web)：**3D 农场建模全面重制**——场景层：深蓝渐变雾夜空 + 650 点星空粒子层（缓慢自转）+ 圆形农场边界光环（青色）+ 半球光（天空蓝/地面棕）+ 暖色平行光（PCFSoft 软阴影，shadow map 1024）+ 渲染器 ACESFilmicToneMapping + sRGB 输出；地块层：梯形双层田垄（0x3a2d1c 垄背/0x241d15 垄沟，行错位起伏）+ 每地块 3×3 植株（相位错开随风摆动）+ 全地块光环（认养紫/未认养灰蓝，opacity 0.4→hover 0.85）；作物按品类：苹果/梨/橘=锥形树干+Icosahedron 椭球树冠+成熟挂果，草莓=锥形叶丛+红果+白花，番茄=支架+Torus 弧段藤蔓+垂挂果，麦稻=9~12 秆簇+金穗（成熟低头），玉米=高秆+弧形长叶+棒穗+抽须，其他兜底叶丛；数据驱动：生长进度 scale 0.25→1.0，健康度 farm3dLeafColor lerpColors（0xb8a24a 枯黄→0x2f6d33 深绿，实测端点精确），pct≥100 果实显现+呼吸动画；交互：Raycaster hover 叶面 emissive 0.12→0.45、点击实时数据卡；性能：几何/材质按品类 Map 池 + clone 共享材质、作物组差量增删。线上实测：6 地块渲染正常（当前健康度 20~44，枯黄矮小为数据驱动正确表现） |
| v16.8 | 2026-09-02 | `093f740` | feat(api+web)：**市场仓库**——收获不再直发积分，而是存入玩家仓库（按 0.5-4 正态随机游走钟，每 30s 推一档，全局持久化均值 2.25、波动 ≈0.16）；新增 4 个端点 `GET /api/v1/market`（当前倍率+滚动历史+18 作物基础价格公示）、`GET /api/v1/warehouse`（我的库存与余额）、`POST /api/v1/warehouse/sell` / `buy`（基础价×倍率，带库存与积分双侧 409 守卫）；积分余额 = 历史收获 + 交易流水净额，总榜同口径；前端报告页追加市场仓库板块：倍率走势 Canvas 曲线（0.5-4 网格+均值虚线+涨红跌绿+面积填充）、基础价格公示表（每行买入按钮）、我的仓库卡（空态引导+出售按钮+30s 定时刷新页面可见才拉）；前端 `bindMarketActions` 事件委托处理 prompt 弹窗买卖；收获响应附带 `face_value` 参考价值与"已收入仓库"提示文案 |
| v16.9 | 2026-09-02 | `16e9d40` | feat(api+web)：**认养生长提速 5×**——加速钟原为 1 分钟 = 1 天（100 天作物要 100 分钟才熟，反馈偏慢）；存量认养 `time_scale` 全量 ×5（cap 60，线上已迁移，pct 立即按 5 倍反映），新认养 INSERT 默认 `time_scale=5`（1 分钟 = 5 天，100 天作物约 20 分钟成熟）；认养页空态文案同步 5× 加速钟 + 仓库售卖引导；PWA 缓存 v21→v22。加速系数仍可被管理员 ⏩ 调整（1-60×） |
